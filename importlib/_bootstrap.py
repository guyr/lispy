...
class _LoaderBasics:
    ...
    def _bytes_from_bytecode(self, fullname, data, source_mtime):
        ...
        if len(magic) != 4 or magic != imp.get_magic():
            raise ImportError("bad magic number in {0}".format(fullname))
        elif len(raw_timestamp) != 4:
            raise EOFError("bad timestamp in {0}".format(fullname))
        elif source_mtime is not None:
            if marshal._r_long(raw_timestamp) != source_mtime:
                raise ImportError("bytecode is stale for {0}".format(fullname))
...
class SourceLoader(_LoaderBasics):
    ...
    # TODO: if the mixin idea will be retained, this should not exist
    #       here at all to avoid masking the mixin
    ...
    def compile_source(self, fullname, source_bytes, source_path):
        # must be implemented for loading uncompiled source files
        raise NotImplementedError
    ...
    def get_source(self, fullname):
        ...
        try:
            source = self.decode_source(source_bytes)
        except NotImplementedError:
            # XXX not sure if this is OK
            return source_bytes
        return source

    def get_cached_code(self, fullname):
                ...
                try:
                    ...
                    try:
                        ...
                        if isinstance(found, code_type):
                            found = _fix_co_filename(found, source_path)
                            # this is a way to enable loaders to force re-
                            # compilation (in case of dependency changes)
                            try:
                                if self.verify_cache(found):
                                    return found
                            except NotImplementedError:
                                return found
                        else:
                            msg = "Non-code object in {0}"
    ...
    def get_code(self, fullname):
        ...
        """
        code_object = self.get_cached_code(fullname)
        if code_object:
            return code_object
        ...
        code_object = self.compile_source(fullname, source_bytes, source_path)
...
class PySourceLoader(SourceLoader):

    def decode_source(self, source_bytes):
        import tokenize
        # TODO: implement detect_encoding for py2.7
        # TODO?: respond properly when tokenize.detect_encoding fails
...
class _SourceFileLoader(FileLoaderMixin, PySourceLoader):

    """Concrete implementation of SourceLoader using the file system."""


class _SourcelessFileLoader(FileLoaderMixin, _LoaderBasics):
    ...
    def get_code(self, fullname):
        ...
        if isinstance(found, code_type):
            ...
            raise ImportError("Non-code object in {0}".format(path))
    ...
    def find_module(self, fullname):
        ...
        if _path_isdir(base_path) and _case_ok(self.path, tail_module):
            for suffix, loader in self.packages:
                ...
                msg = "Not importing directory {0}: missing __init__"
...
def _SourceFinderDetails():
    return {
        "suffixes": _suffix_list(imp.PY_SOURCE),
        "loader": _SourceFileLoader,
        "supports_packages": True}

def _SourcelessFinderDetails():
    return {
        "suffixes": _suffix_list(imp.PY_COMPILED),
        "loader": _SourcelessFileLoader,
        "supports_packages": True}

def _ExtensionFinderDetails():
    return {
        "suffixes": _suffix_list(imp.C_EXTENSION),
...
def _file_path_hook(path):
    ...
    if _path_isdir(path):
        if hasattr(sys, "import_suffix_hooks"):
            details = sys.import_suffix_hooks[:]
        else:
            details = []
        details += [_ExtensionFinderDetails(), _SourceFinderDetails(),
            _SourcelessFinderDetails()]
        return _FileFinder(path, *details)
    else:
    ...
    with _ImportLockContext():
        ...
        if parent:
            ...
            try:
                ...
                msg = (_ERR_MSG + '; {1} is not a package').format(name, parent)
